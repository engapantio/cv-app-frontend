...
describe("LoginForm", () => {
  it("renders the form with title, email, password, submit button and forgot-password link", () => {
    render(<LoginForm />);
  ...
  it("calls fetch with the correct payload on submit", async () => {
    ...
    await user.type(screen.getByLabelText("passwordLabel"), "secret123");
    await user.click(screen.getByRole("button", { name: "submit" }));
  ...
  it("disables fields while submitting", async () => {
    ...
    expect(screen.getByRole("button", { name: "submitLoading" })).toBeDisabled();
  ...
  it("shows the API message when payload.user is missing", async () => {
    fetchMock.mockResolvedValue({
      ok: false,
